    ...
    public static class DbSeed
    {
        public static void SeedRolesAndUsers()
        {
            using (var context = new ApplicationDbContext())
            {
                ...
                if (!context.Users.Any(x => x.UserName == "admin@emineyenice.com"))
                {
                    var userStore = new UserStore<ApplicationUser>(context);
                    var userManager = new UserManager<ApplicationUser>(userStore);
                    var adminUser = new ApplicationUser()
                    {
                        UserName = "admin@emineyenice.com",
                        Email = "admin@emineyenice.com",
                        EmailConfirmed = true
        ...
        public async static Task SeedMovies()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //http://www.omdbapi.com/?apikey=4bdb55c1&i=tt0111162
                client.BaseAddress = new Uri("http://www.omdbapi.com/");

                foreach (string imdbId in ImdbMovieIds())
                {
                    var result = await client.GetAsync("?apikey=4bdb55c1&i="+ imdbId);

                    if (result.IsSuccessStatusCode)
                    {
                        //install-package Microsoft.AspNet.WebApi.Client api'lere ulasmayı sağlayan kütüphane
